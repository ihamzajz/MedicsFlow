...
$transport_name = trim($_GET['transport_name'] ?? '');
...
$hasAnyFilter = ($month!=='' || $year!=='' || $date_from!=='' || $date_to!=='' || $search!=='' || $transport_name!=='');
...
if (!$errors) {
    ...
    if ($transport_name !== '') {
        ...
        $types .= "s";
        $params[] = $transport_name;
    }

    // Search
    if ($search !== '') {
        $where[] = "(transport_name LIKE ? OR `day` LIKE ? OR time_in LIKE ? OR time_out LIKE ? OR form_user LIKE ?)";
        $types .= "sssss";
        $like = "%{$search}%";
        $params[] = $like;
        $params[] = $like;
...
function fetchAllFilteredRows($conn, $whereSql, $types, $params) {
    $sql = "
        SELECT id, `date`, `day`, transport_name, time_in, time_out, form_user
        FROM transport_form
        {$whereSql}
...
/* ----------------- Report title parts ----------------- */
$parts = [];
if ($transport_name !== '') $parts[] = "Transport: " . $transport_name;
...
$reportTitle = "Transport Record List" . ($parts ? " (" . implode(", ", $parts) . ")" : "");
...
if ($transport_name !== '') {
    // keep it simple: show transport selected
    $showingHtml = 'Showing data for <strong>'.h($transport_name).'</strong>';
    if ($date_from !== '' && $date_to !== '') {
        $df = formatDateLabel($date_from, $monthsMap);
        $dt = formatDateLabel($date_to, $monthsMap);
        $showingHtml .= ' from <strong>'.h($df).'</strong> to <strong>'.h($dt).'</strong>';
    }
}
...
if (!$errors && $export === 'excel') {
    ...
    $filename = "transport_record_list_" . date("Y-m-d_H-i") . ".csv";
    ...
    fputcsv($out, []);
    fputcsv($out, ["Ref#", "Date", "Day", "Transport Name", "Time In", "Time Out", "User"]);
...
if (!$errors) {
    // Count
    $stmtC = $conn->prepare("SELECT COUNT(*) FROM transport_form {$whereSql}");
...
<head>
    ...
    <title>Transport Record List</title>
                                ...
                                <form method="GET">
                                    ...
                                    <div class="row g-2 align-items-end">
                                        ...
                                        <div class="col-12 col-md-2">
                                            ...
                                            <select class="form-select" name="transport_name">
                                                <option value="">All</option>
                                                <?php foreach ($transportOptions as $t): ?>
                                                    ...
                                                    ?>
                                                    <option value="<?php echo h($nm); ?>" <?php echo ($transport_name===$nm?'selected':''); ?>>
                                                        <?php echo h($label); ?>
                                                    </option>
                                                <?php endforeach; ?>
                                            </select>
                                        ...
                                        <div class="col-12 col-md-1">
                                            ...
                                            <input type="text" class="form-control" name="search"
                                                   placeholder="name, day, time, user..."
                                    ...
                                    <div class="filter-actions">
                                        ...
                                        <div class="right">
                                            <?php if ($hasFilter): ?>
                                                <a class="btn btn-sm btn-sq btn-clear" href="transport_record_list.php">
                                    ...
                                    <div class="table-responsive">
                                        <table class="table table-hover align-middle">
                                            ...
                                            <tr>
                                                ...
                                                <th style="width:240px;">Transport Name</th>
                                            ...
                                            <?php else: ?>
                                                <?php foreach ($rows as $r): ?>
                                                    <tr>
                                                        ...
                                                        <td><?php echo h($r['transport_name']); ?></td>
                                                        ...
                                                        <td>
                                                            <a class="btn btn-sm btn-sq btn-edit"
                                                               href="transport_record_edit.php?id=<?php echo (int)$r['id']; ?>">
...
function downloadPDF() {
    ...
    doc.autoTable({
        startY: 20,
        head: [[ "Ref#", "Date", "Day", "Transport Name", "Time In", "Time Out", "User" ]],
        body: EXPORT_ROWS,
        styles: { fontSize: 8 },
        headStyles: { fillColor: [33, 37, 41], textColor: 255 }
    });

    doc.save("transport_record_list.pdf");
}
</script>
