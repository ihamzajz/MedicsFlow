<?php 
    ...
    $all_count_query = "SELECT COUNT(*) AS all_count FROM raheel1 WHERE product_name = 'DIGAS SYRUP 120ML'";
...
<html lang="en">
    <head>
        ...
        <style>
            #pagination-controls-unique {
    text-align: center;
    margin-top: 20px;
}

.pagination-button-unique {
    margin: 0 5px;
    padding: 5px 10px;
...
.pagination-button-unique:hover {
    ...
    <div class="container-fluid">
        ...
        <nav>
            <div class="nav nav-tabs" id="nav-tab" role="tablist">
                <button class="nav-link" id="nav-disabled-tab" data-bs-toggle="tab" data-bs-target="#nav-disabled" type="button" role="tab" aria-controls="nav-disabled" aria-selected="false" disabled>Digas Syrup 120ML</button>
                ...
                <button class="nav-link" id="nav-profile-tab" data-bs-toggle="tab" data-bs-target="#nav-profile" type="button" role="tab" aria-controls="nav-profile" aria-selected="false">All Data</button>
        ...
        <div class="tab-content" id="nav-tabContent">
            ...
            <div class="tab-pane fade show active" id="nav-home" role="tabpanel" aria-labelledby="nav-home-tab" tabindex="0">
                ...
                <div class="row justify-content-center">
                    <div class="col-md-5 ">
                        <input type="text" id="searchInput" class="form-control mt-2 mb-0" placeholder="Search by Purchase Quantity" oninput="filterData()" style="font-size:13px;border-radius:0px;width:100%">
                    </div>
                </div>
                <div class="table-wrapper row justify-content-center mt-0 pt-0">
                    <div class="table-responsive table-container col-md-5">
                        <!-- Search Field -->
                ...
                <div class="pagination-controls text-center mt-3 mb-2">
                    <button id="prevPage" class="btn btn-dark btn-sm" style="border-radius:0px" onclick="prevPage()">Previous</button>
                    <span id="pageInfo" class="mx-3"></span>
                    <button id="nextPage" class="btn btn-dark btn-sm" style="border-radius:0px" onclick="nextPage()">Next</button>
                </div>
                <script>
                    ...
                    function renderTable(page) {
                        ...
                        for (let i = start; i < end; i++) {
                            const row = `<tr><td>${filteredData[i].purchaseQuantity}</td><td>${filteredData[i].freeProducts}</td></tr>`;
                    ...
                    function filterData() {
                        ...
                        filteredData = data.filter(item => item.purchaseQuantity.toString().includes(searchInput));
                        totalPages = Math.ceil(filteredData.length / recordsPerPage);
            ...
            <div class="tab-pane fade" id="nav-profile" role="tabpanel" aria-labelledby="nav-profile-tab" tabindex="0">
                ...
                <h5 class="pt-3">
                    ...
                    <input id="filter2" type="text" class="form-control" placeholder="Search here..." style="font-size: 11px; border-radius: 4px; padding: 6px 10px; width: 200px; height: auto; display: inline-block;color:black">
                    </span>
                </h5>
                <?php
                    include 'dbconfig.php';
                    // Initial query (optional) or leave this out if you only load data via AJAX
                    $select = "SELECT * FROM raheel1 WHERE product_name = 'DIGAS SYRUP 120ML'";
            ...
            <div class="tab-pane fade" id="nav-group" role="tabpanel" aria-labelledby="nav-group-tab" tabindex="0">
                <!-- b start -->
                <h5 class="pt-3">Group Invoice
                    <span style='float:right!important'>
                    <input id="filter3" type="text" class="form-control" placeholder="Search here..." style="font-size: 11px; border-radius: 4px; padding: 6px 10px; width: 200px; height: auto; display: inline-block;color:black">
                    </span>
                </h5>
                <div class="table-wrapper">
                    <div class="table-responsive table-container">
                        <table class="table table-striped" id="myTable3">
                            <!-- Unique ID for new table -->
                            <thead>
                                <tr>
                                    <th>Depot Name</th>
                                    <th>Inv #</th>
                            ...
                            <tbody id="data-body-group">
                                <!-- Unique ID for new table body -->
                                <!-- Data will be loaded via AJAX -->
                            </tbody>
                        </table>
                    </div>
            ...
            <div id="pagination-controls-group" class="text-center">
            ...
            <div class="tab-pane fade" id="nav-contact" role="tabpanel" aria-labelledby="nav-contact-tab" tabindex="0">
                ...
                <?php
                    ...
                    if (!empty($discrepancies)) {
                        ...
                        echo "<h5 class='pt-2'>Discrepancies Found: $discrepancyCount
                        <span style='float:right!important'>
                        <button id='excel4' class='btn btn-success btn-sm dataExport' data-type='excel'>Excel</button>
                    <input id='filter4' type='text' class='form-control' placeholder='Search here...' style='font-size: 11px; border-radius: 4px; padding: 6px 10px; width: 200px; height: auto; display: inline-block;color:black'>
                        </span></h5>";
                    
                        echo "<div class='table-wrapper'>";
                        echo "<div class='table-responsive table-container1' >";
    ...
    <script>
        $(document).ready(function() {
            ...
            function loadData() {
                $.ajax({
                    url: 'fetchData_ds.php', // Adjust this URL to your backend script
                    type: 'GET',
                    data: {
                        limit: limit,
                        offset: offset,
    ...
    <script>
        ...
        let offsetGroup = 0;
        const limitGroup = 15;
        const $dataBodyGroup = $('#data-body-group');
        const $paginationControlsGroup = $('#pagination-controls-group');
        const $filterInputGroup = $('#filter3');
        ...
        function loadDataGroup() {
            $.ajax({
                ...
                data: {
                    limit: limitGroup,
                    offset: offsetGroup,
                    search: $filterInputGroup.val()
                },
                dataType: 'json',
                success: function(response) {
                    if (response.data) {
                        $dataBodyGroup.html(response.data.rows);
                        $paginationControlsGroup.html(response.data.pagination);
                    }
                }
            });
        }
        ...
        $paginationControlsGroup.on('click', 'button', function() {
            let newOffset = $(this).data('offset');
            if (newOffset !== undefined) {
                offsetGroup = newOffset;
                loadDataGroup();
            }
        });
        
        $filterInputGroup.on('input', function() {
            offsetGroup = 0;
            loadDataGroup();
        });
        });
