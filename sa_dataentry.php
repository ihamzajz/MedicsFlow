...
if (isset($_GET['ajax'])) {
    ...
    if ($_GET['ajax'] === 'roles') {
        ...
        $productSafe = $product; // using prepared stmt
        $sql = "SELECT role
            FROM staff_allo_pro
...
/* =========================================================
   AJAX submit (NO reload) + Toast response (Update/Complete)
========================================================= */
if ($_SERVER['REQUEST_METHOD'] === 'POST' && (isset($_POST['submit']) || isset($_POST['complete']))) {
    ...
    if (isset($_POST['complete'])) {
        $up = mysqli_prepare($conn, "UPDATE staff_allocation SET complete_status='completed' WHERE id=?");
        ...
        if ($isAjax) {
            echo json_encode([
                ...
                "msg" => $ok ? "Form has been completed." : "Complete failed. Please try again."
            ]);
            exit;
        }
        ...
        // fallback (non-ajax)
        if ($ok) {
            echo "<script>alert('Form has been completed.'); window.location.href='sa_dataentry_list.php';</script>";
        } else {
            echo "<script>alert('Update Failed.'); window.location.href=window.location.href;</script>";
        }
        exit;
    }
...
========================================================= */
        ...
        /* Remove Bootstrap size overrides */
        .form-control-sm,
        .form-select-sm {
            height: 28px !important;
            min-height: 28px !important;
...
<body>
                ...
                <div class="container-xxl">
                    ...
                    <div class="card shadow-sm border-0 mb-3 topbar-card">
                        <div class="card-body">
                            ...
                            <a href="sa_dataentry_list" class="btn btn-secondary btn-sm">
                                <i class="fa-solid fa-list me-1"></i> Back
                            </a>

                            <h5 class="title-center">Staff Allocation - Data Input</h5>
                    ...
                    <form method="POST" id="staffForm">
                                        ...
                                        <tbody>
                                            <?php for ($i = 1; $i <= 40; $i++): ?>
                                                ...
                                                <tr id="row_<?= $i ?>" class="<?= $hiddenClass ?>" data-row="<?= $i ?>">
                                                    ...
                                                    <td class="col-name">
                                                        <!-- AJAX select2: keep current selected as single option (no huge list) -->
                                                    ...
                                                    <td class="col-jobs">
                                                        <!-- AJAX select2 roles: keep current selected as single option -->
                        ...
                        <div class="card shadow-sm border-0 mb-3">
                            <div class="card-body d-flex justify-content-center gap-2 flex-wrap">
                                ...
                                <button type="submit" name="complete" class="btn btn-secondary btn-sm">
                                    <i class="fa-regular fa-circle-check me-1"></i> Complete Form
                                </button>
                            </div>
                        </div>
    ...
    <script>
        ...
        // Clear row (clears Select2 properly)
        function clearRow(i) {
            ...
            row.querySelectorAll('select').forEach(sel => {
                ...
                // remove all non-empty options (removes the preselected option from PHP)
        ...
        function initSelect2ForRow(i) {
            ...
            if ($name.length && !$name.data('select2')) {
                $name.select2({
                    ...
                    ajax: {
                        url: window.location.pathname + window.location.search.replace(/([&?])ajax=[^&]*/g, '').replace(/[?&]$/, '') + (window.location.search.includes('?') ? '&' : '?') + 'ajax=users',
                        dataType: 'json',
                        delay: 250,
            ...
            if ($jobs.length && !$jobs.data('select2')) {
                $jobs.select2({
                    ...
                    ajax: {
                        url: function() {
                            const base = window.location.pathname + window.location.search.replace(/([&?])ajax=[^&]*/g, '').replace(/[?&]$/, '');
        ...
        $(document).ready(function() {
            ...
            let lastAction = null;
            $('button[type="submit"][name="submit"], button[type="submit"][name="complete"]').on('click', function() {
                lastAction = $(this).attr('name'); // "submit" or "complete"
            });
            ...
            $('#staffForm').on('submit', function(e) {
                ...
                if (lastAction) fd.set(lastAction, '1');

                // tell PHP this is AJAX
                ...
                fetch(window.location.href, {
                    ...
                    .then(data => {
                        if (data && data.ok) {
                            showToast(data.type || 'success', data.title || 'Success', data.msg || '');
                        } else {
