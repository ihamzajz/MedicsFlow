...
<body>
    ...
    <div class="wrapper d-flex align-items-stretch">
        ...
        <div id="content">
            ...
            <a class="btn btn-dark btn-sm mx-2" href="expense_home.php" style="font-size:11px!important"><i class="fa-solid fa-arrow-left"></i> Home</a>

            <?php
            include 'dbconfig.php';
            $select = "SELECT * FROM cash_purchase where status = 'Open'";
                            ...
                            <tbody class="searchable">
                                <tr id="row_<?php echo $row['id']; ?>">
                                    ...
                                    <td>
                                        ...
                                        <a href="cash_purchase_head_details.php?id=<?php echo $row['id']; ?>" class="btn btn-fixed-text">
                                            <i class="fa-solid fa-arrow-up"></i> View Details
                                        </a>
                                    </td>
                                </tr>
                            </tbody>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            $('#excel-general').click(function() {
                $('#myTable-general').tableExport({
                    type: 'excel',
                    ignoreColumn: [16]
                });
            });
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            $('#excel-mb').click(function() {
                $('#myTable-mb').tableExport({
                    type: 'excel',
                    ignoreColumn: [16]
                });
            });
    ...
    <script>
        $(document).ready(function() {
            (function($) {
                $('#filter-general').keyup(function() {
                    var rex = new RegExp($(this).val(), 'i');
                    $('.searchable-general tr').hide();
                    $('.searchable-general tr').filter(function() {
                        return rex.test($(this).text());
                    }).show();
                })
    ...
    <script>
        $(document).ready(function() {
            (function($) {
                $('#filter-mb').keyup(function() {
                    var rex = new RegExp($(this).val(), 'i');
                    $('.searchable-mb tr').hide();
                    $('.searchable-mb tr').filter(function() {
                        return rex.test($(this).text());
                    }).show();
                })
    ...
    <!-- TABS BOOTSTRAP CODE -->
    <script>
        // Selecting the tabs and their corresponding triggers
        ...
        function activateTab(tabId) {
            ...
            selectedTabContent.classList.add('active', 'show');
        }

        // Event listener to handle tab clicks
        tabs.forEach(function(tab) {
            tab.addEventListener('click', function() {
                var tabId = this.getAttribute('data-bs-target').slice(1); // Extracting tab id
                activateTab(tabId);
            });
        });

        // Activating the first tab by default
