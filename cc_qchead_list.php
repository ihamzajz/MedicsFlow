...
$select = "SELECT * FROM qc_ccrf 
           ...
           AND ('$be_depart' = 'super' OR user_be_role = 'approver' OR dept_head_status = 'Approved')
           ORDER BY code DESC";
...
<div class="d-flex align-items-center justify-content-between">
    ...
    <h6 class="m-0 text-center" style="flex:1;font-weight:600">Quality Head Approval</h6>
        ...
        ?>
                <tbody class="searchable">
                    <tr id="row_<?php echo $row['id']; ?>">
                        <td>
                            <a href="#" class="btn btn-approve" onclick="promptReason2(
                                        <?php echo $row['id']; ?>, 
                                        '<?php echo htmlspecialchars($row['user_email'], ENT_QUOTES, 'UTF-8'); ?>', 
                                        '<?php echo htmlspecialchars($row['user_name'], ENT_QUOTES, 'UTF-8'); ?>')">
                                <i class="fa-regular fa-circle-check"></i> Approve
                            </a>
                        <td><a href="#" class="btn btn-reject" onclick="promptReason(
                                    <?php echo $row['id']; ?>, 
                                    '<?php echo htmlspecialchars($row['user_email'], ENT_QUOTES, 'UTF-8'); ?>', 
                                    '<?php echo htmlspecialchars($row['user_name'], ENT_QUOTES, 'UTF-8'); ?>')">
                        ...
                        <!-- <td>
                            <a href="cc_dashboard_details.php?id=<?php echo $row['id']; ?>" class="btn btn-fixed-text"
                                style="font-size:11px!important;font-weight:500!important;letter-spacing:0.5px!important;background-color:#129990;border-radius:15px!important">
...
<script>
    function promptReason(itemId, userEmail, userName) {
        var reason = prompt("Enter Remarks:");
        if (reason != null && reason.trim() !== "") {
            window.location.href = "cc_qchead_reject.php?id=" + itemId +
...
<script>
    function promptReason2(itemId, userEmail, userName) {
        var reason = prompt("Enter Remarks:");
        if (reason != null && reason.trim() !== "") {
            window.location.href = "cc_qchead_approve.php?id=" + itemId +
                "&reason=" + encodeURIComponent(reason) +
                "&user_email=" + encodeURIComponent(userEmail) +
                "&user_name=" + encodeURIComponent(userName);
        }
