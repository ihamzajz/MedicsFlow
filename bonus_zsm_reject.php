...
$reason = $_GET['reason'];
$update="UPDATE bonus_form SET zsm_status = 'Rejected' ,zsm_msg = 'Rejected By $approver_name',zsm_date = '$date' , task_status='Rejected By $approver_name', final_status = 'Rejected', reason = '$reason' WHERE id = $id";
...
try {
    ...
    $mail->AltBody = "Hi $user_name Your bonus request has been approved by $approver_name";
    $mail->send();
    header("Location:bonus_zsm_list.php");
    $mail->clearAddresses(); 
    $mail->send();
    header("Location:bonus_zsmasm_list.php");
} catch (Exception $e) {
