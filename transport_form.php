...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax']) && $_POST['ajax'] === '1') {
    ...
    $activeGuards = [];
    $res = $conn->query("SELECT transport_name FROM transport_guard WHERE status = 1 ORDER BY transport_name ASC");
    if ($res) {
        while ($row = $res->fetch_assoc()) {
            $activeGuards[] = $row['transport_name'];
        }
        $res->free();
    }

    if (count($activeGuards) === 0) {
        echo json_encode(['ok'=>false,'msg'=>'No active transport found. Please add transport guards first.']);
        exit;
    }
    ...
    // RULES:
    // - In/Out are optional (user can fill one or both or none)
    // - Each must be valid if provided
    // - If both provided, out >= in
    // - Save row if at least one of in/out is provided for that transport
    $rowsToInsert = [];
    foreach ($activeGuards as $name) {
        $in  = isset($t_in[$name])  ? trim($t_in[$name])  : '';
        $out = isset($t_out[$name]) ? trim($t_out[$name]) : '';
        ...
        // Save as separate record per transport (only if user entered at least one time)
        if ($in !== '' || $out !== '') {
            $rowsToInsert[] = [$date, $day_to_save, $name, $in, $out, $form_user];
        }
    }
    ...
    if (!$ok) {
        echo json_encode(['ok'=>false,'msg'=>'Please enter at least one transport time. (Invalid time or Out earlier than In is not allowed)']);
        exit;
    }

    // Insert rows (multiple records) INCLUDING day column
    $stmt = $conn->prepare("
        INSERT INTO transport_form (`date`, `day`, transport_name, time_in, time_out, form_user)
        VALUES (?, ?, ?, ?, ?, ?)
    ");
    ...
    try {
        ...
        echo json_encode(['ok'=>true,'msg'=>'Transport form has been submitted!']);
        exit;
...
$today_day = date('l');         // Day name for UI

// Fetch active guards for showing in UI
$guards = [];
$res = $conn->query("SELECT id, transport_name FROM transport_guard WHERE status = 1 ORDER BY transport_name ASC");
if ($res) {
...
<head>
    ...
    <style>
        ...
        /* ===== Interactive Row UI (1 line per transport) ===== */
        .t-row{
            background: #ffffff;
            border: 1px solid #e5e7eb;
            border-radius: 12px;
        ...
        <div class="container-fluid">
            <div class="row justify-content-center mt-5">
                <div class="col-md-10 pt-md-2">
                    ...
                    <form class="form pb-3" method="POST" autocomplete="off" id="transportForm">
                        <div class="card shadow">
                            <div class="card-header bg-header text-white d-flex justify-content-between align-items-center">
                                <h6 class="mb-0">Transport Form</h6>
                                <a href="transport_home.php" class="btn btn-light btn-sm">
                            ...
                            <div class="card-body">
                                ...
                                <?php if (count($guards) === 0): ?>
                                    <div class="alert alert-warning mb-0">
                                        No active transport found. Please add records in <b>transport_guard</b> (status = 1).
                                    </div>
                                <?php else: ?>

                                    <div class="d-flex flex-column row-gap-compact">
                                        <?php $num = 1; foreach ($guards as $g):
                                            $name = $g['transport_name'];
                                            $id = (int)$g['id'];
                                        ?>
                                            <div class="col-12">
                                                <div class="t-row">
                                                    ...
                                                    <div class="t-right">
                                                        <div class="time-group">
                                                            ...
                                                            <input
                                                                class="time-input"
                                                                type="time"
                                                                name="time_in[<?php echo htmlspecialchars($name); ?>]"
                                                                id="time_in_<?php echo $id; ?>"
                                                            >
                                                        </div>
                                                        ...
                                                        <div class="time-group">
                                                            ...
                                                            <input
                                                                class="time-input"
                                                                type="time"
                                                                name="time_out[<?php echo htmlspecialchars($name); ?>]"
                                                                id="time_out_<?php echo $id; ?>"
                                                            >
                                                        </div>
                                                    </div>
                                                </div>
                                                ...
                                                <div class="t-errors">
                                                    <small class="text-danger d-none" id="err_in_<?php echo $id; ?>"></small>
                                                    <small class="text-danger d-none" id="err_out_<?php echo $id; ?>"></small>
                                                </div>
                                            </div>
                                        <?php $num++; endforeach; ?>
                                    </div>
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    // Validate HH:MM (time input already helps, but keep logic)
    ...
    function validateAll(){
        ...
        inInputs.forEach(inEl => {
            ...
            const key = nameKey.substring(8, nameKey.length - 1);
        ...
        const anyFilled = Array.from(form.querySelectorAll('input[type="time"]'))
            .some(x => (x.value || "").trim() !== "");
        if (!anyFilled) {
            showToast("toast-warning", "Please enter at least one transport time.");
            ok = false;
        }
    ...
    form.addEventListener("submit", async function(e) {
        ...
        try {
            ...
            if (data.ok) {
                showToast("toast-success", data.msg || "Transport form has been submitted!");
