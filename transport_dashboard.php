...
/**
 * ✅ NEW: format time to 12-hour AM/PM
 * Accepts:
 *  - "HH:MM"
 *  - "HH:MM:SS"
 * Returns:
 *  - "h:i A"  e.g. "12:05 AM", "1:30 PM"
 * If empty/null => ""
 */
function formatTime12($t): string {
    $t = trim((string)$t);
    if ($t === '') return '';
    // Normalize seconds if not present
    if (preg_match('/^\d{2}:\d{2}$/', $t)) $t .= ':00';

    $dt = DateTime::createFromFormat('H:i:s', $t);
    if (!$dt) return $t; // fallback if something unexpected
...
/* ----------------- Transport Name dropdown (ALL status) ----------------- */
$transportOptions = [];
...
$reportTitle = "Transport Report" . ($parts ? " (" . implode(", ", $parts) . ")" : "");
...
if (!$errors && $export === 'excel') {
    ...
    $filename = "transport_report_" . date("Y-m-d_H-i") . ".csv";
    ...
    foreach ($exportRows as $r) {
        fputcsv($out, [
            ...
            formatTime12((string)$r['time_in']),   // ✅ 12-hour
            formatTime12((string)$r['time_out']),  // ✅ 12-hour
            (string)$r['form_user'],
        ]);
    }
                            ...
                            <div class="panel panel-pad">
                                ...
                                <form method="GET">
                                    ...
                                    <div class="filter-actions">
                                        ...
                                        <div class="right">
                                            <?php if ($hasFilter): ?>
                                                <a class="btn btn-sm btn-sq btn-clear" href="transport_dashboard.php">
