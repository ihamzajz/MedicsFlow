...
/* =========================
   Session (same style as your workorder pages)
========================= */
$id         = $_SESSION['id'] ?? '';
...
/* =========================================================
   ✅ Server-side Pagination + Search + Dept + Date Range
   Base: hr_status = Pending
   Date filter is on submit date column: `date`
========================================================= */
$perPage = 50;
...
if ($qRaw !== '') {
  ...
  $whereParts[] = "(
      ...
      OR reporting_to LIKE '{$like}'
      OR head_of_department LIKE '{$like}'
      OR date LIKE '{$like}'
      OR start_date LIKE '{$like}'
  )";
}

/* Date range validation + filter (SUBMIT DATE) */
$hasFrom = ($dateFrom !== '');
$hasTo   = ($dateTo !== '');
...
/* Count */
$countSql = "SELECT COUNT(*) AS total FROM newuserform WHERE {$whereSql}";
...
/* Fetch page */
$select = "SELECT * FROM newuserform
           WHERE {$whereSql}
...
<head>
  ...
  <title>New User - HR Approval</title>
  ...
  <style>
    ...
    .td-wrap { max-width: 520px; white-space: normal; word-break: break-word; overflow-wrap: anywhere; }
    .box1-heading { font-weight: 700 !important; }
    a { text-decoration: none !important; }
    label { font-size: 13px !important; }

    input, select { border: 0.5px solid #ced4da !important; font-size: 12px !important; }
    ...
    .mint-badge i { color: #0f766e; white-space: nowrap; }
    ...
    .table-responsive.dt-wrap table { width: max-content; min-width: 100%; border-collapse: separate; border-spacing: 0; table-layout: auto; }
    .table-responsive.dt-wrap thead th { position: sticky; top: 0; z-index: 5; }
    .table-responsive.dt-wrap td { white-space: nowrap; background: #fff; }
    .table-responsive.dt-wrap td.td-wrap { white-space: normal !important; max-width: 520px; word-break: break-word; overflow-wrap: anywhere; }
    .table-responsive.dt-wrap tbody tr:hover td { background: #f8fbff; }
    .table-responsive.dt-wrap tbody tr:nth-child(even) td { background: #fbfdff; }
    .table-responsive.dt-wrap tbody td { border-bottom: 1px solid #e5e7eb !important; }
        ...
        <div class="container-xxl">
          ...
          <div class="card shadow-sm border-0 mb-3">
            <div class="card-body d-flex align-items-center justify-content-between flex-wrap gap-2">
              <div>
                <h5 class="mb-0 box1-heading">New User - HR Approval</h5>
              </div>

              <a href="new_user_home.php" class="btn btn-dark btn-sm">
              ...
              <form id="searchForm" method="GET" action="" class="row g-2 align-items-end">
                ...
                <div class="col-6 col-lg-1 d-grid">
                  <button
                    ...
                    style="<?php echo (($qRaw === '' && $deptRaw === '' && $dateFrom === '' && $dateTo === '') ? 'display:none;' : ''); ?>">
                  ...
                  <tbody>
                    ...
                    if ($data) {
                      while ($row = mysqli_fetch_assoc($select_q)) {
                        ...
                        // ✅ Submit date (your form submit date column)
                        $submitDatePretty = '';
                        if (!empty($row['date'])) {
                          try { $submitDatePretty = (new DateTime($row['date']))->format('d-m-Y'); }
                          catch (Exception $e) { $submitDatePretty = (string)$row['date']; }
                        }

                        // ✅ Start date (from start_date column)
                        $startDatePretty = '';
                        if (!empty($row['start_date'])) {
                          try { $startDatePretty = (new DateTime($row['start_date']))->format('d-m-Y'); }
                          catch (Exception $e) { $startDatePretty = (string)$row['start_date']; }
                        }
                    ?>
                        <tr id="row_<?php echo $rid; ?>">
                          <td style="white-space:nowrap;">
                            <a href="newuser_hr_details.php?id=<?php echo $rid; ?>" class="btn-details">
                              <i class="fa-solid fa-arrow-up me-1"></i> Details
                            </a>
                          </td>

                          <td><?php echo htmlspecialchars((string)$row['id']); ?></td>
                          <td class="td-wrap"><?php echo htmlspecialchars((string)$row['emp_id']); ?></td>
                          <td class="td-wrap"><?php echo htmlspecialchars((string)$row['name']); ?></td>
                          <td style="white-space:nowrap;"><?php echo htmlspecialchars($submitDatePretty); ?></td>
                          <td class="td-wrap"><?php echo htmlspecialchars((string)$row['department']); ?></td>
                          <td class="td-wrap"><?php echo htmlspecialchars((string)$row['position']); ?></td>
                          <td class="td-wrap"><?php echo htmlspecialchars((string)$row['reporting_to']); ?></td>
                          <td class="td-wrap"><?php echo htmlspecialchars((string)$row['head_of_department']); ?></td>
                          <td style="white-space:nowrap;"><?php echo htmlspecialchars($startDatePretty); ?></td>
                        </tr>
                    <?php
                      }
                    } else {
    ...
    <script>
      document.addEventListener("DOMContentLoaded", function() {
        ...
        form.addEventListener("submit", function(e) {
          ...
          if (dp === "") url.searchParams.delete("dept"); else url.searchParams.set("dept", dp);
          if (df === "") url.searchParams.delete("dateFrom"); else url.searchParams.set("dateFrom", df);
          if (dt === "") url.searchParams.delete("dateTo"); else url.searchParams.set("dateTo", dt);
