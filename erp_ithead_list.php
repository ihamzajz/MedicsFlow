...
/* =========================================================
   ✅ IT Head List: Server-side Pagination + Search + Date Range
   ✅ Same UI/filters/pagination as erp_userlist
========================================================= */
$perPage = 50;
...
if ($be_depart === 'super') {
  ...
  // Old behavior for IT Head list:
  $whereParts[] = "department_head_status = 'Approved'";
  $whereParts[] = "`role` != 'Head of department'";
  $whereParts[] = "it_head_status = 'Pending'";
}
...
/* Date range validation + filter (FILTER STILL WORKS ON REAL DB DATE) */
$hasFrom = ($dateFrom !== '');
$hasTo   = ($dateTo !== '');
...
<head>
  ...
  <title>ERP Access - IT Head Approval</title>
        ...
        <div class="container-xxl">
          ...
          <div class="card shadow-sm border-0 mb-3">
            <div class="card-body d-flex align-items-center justify-content-between flex-wrap gap-2">
              <div>
                <h5 class="mb-0 box1-heading">ERP Access - IT Head Approval</h5>
              </div>
                  ...
                  <tbody>
                    ...
                    if ($data) {
                      while ($r = mysqli_fetch_assoc($select_q)) {
                        $rid = (int)$r['id'];
                        $dateShow = fmt_ddmmyyyy($r['date'] ?? '');
                    ?>
                        <tr id="row_<?php echo $rid; ?>">
                          <td style="white-space:nowrap;">
                            <a href="erp_ithead_details.php?id=<?php echo $rid; ?>" class="btn-details">
                              <i class="fa-solid fa-arrow-up me-1"></i> Details
                            </a>
                          </td>
                          <td><?php echo htmlspecialchars((string)$r['id']); ?></td>
                          <td class="td-wrap"><?php echo htmlspecialchars((string)$r['name']); ?></td>
                          <td class="td-wrap"><?php echo htmlspecialchars((string)$r['department']); ?></td>
                          <td class="td-wrap"><?php echo htmlspecialchars((string)$r['role']); ?></td>
                          <td style="white-space:nowrap;"><?php echo htmlspecialchars($dateShow); ?></td>
                          <td class="td-wrap"><?php echo htmlspecialchars((string)($r['req_type'] ?? '')); ?></td>
                        </tr>
                    <?php
                      }
                    } else {
