...
if ($id <= 0) {
    header("Location: laundary_record_list.php?status=notfound");
    exit;
}

/* ----------------- Fetch existing record ----------------- */
$stmt = $conn->prepare("SELECT id, `date`, gate_pass_no, transport_qty, uniforms_qty, caps_qty, herb_bags_qty, form_user
                        FROM laundary_form
                        WHERE id = ?
                        LIMIT 1");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax']) && $_POST['ajax'] === '1') {
    ...
    // Basic server-side validation (same as laundary_form)
    $ok = true;
    ...
    if ($ok) {
        ...
        if ($stmtU) {
            ...
            $stmtU->bind_param("ssddddsi", $date, $gate_pass_no, $tq, $uq, $cq, $hq, $form_user, $id);
            $exec = $stmtU->execute();
            ...
            if ($exec) {
                if ($affected > 0) {
                    echo json_encode(['ok' => true, 'status' => 'updated', 'msg' => 'Record updated successfully!']);
                    exit;
                } else {
                    echo json_encode(['ok' => true, 'status' => 'nochange', 'msg' => 'No changes were made.']);
                    exit;
                }
            } else {
                echo json_encode(['ok' => false, 'status' => 'failed', 'msg' => 'Update failed!']);
                exit;
            }
        } else {
            echo json_encode(['ok' => false, 'status' => 'failed', 'msg' => 'Update failed!']);
        ...
        echo json_encode(['ok' => false, 'status' => 'invalid', 'msg' => 'Please fill all fields correctly.']);
        exit;
    }
}
...
$val_gate    = (string)$record['gate_pass_no'];
$val_tq      = number_format((float)$record['transport_qty'], 2, '.', '');
$val_uq      = number_format((float)$record['uniforms_qty'], 2, '.', '');
$val_cq      = number_format((float)$record['caps_qty'], 2, '.', '');
$val_hq      = number_format((float)$record['herb_bags_qty'], 2, '.', '');
?>
<!DOCTYPE html>
<html>
...
<body>
            ...
            <div class="container-fluid">
                <div class="row justify-content-center mt-5">
                    <div class="col-md-8 pt-md-2">
                        ...
                        <form class="form pb-3" method="POST" autocomplete="off" id="laundaryEditForm">
                            <div class="card shadow">
                                <div class="card-header bg-header text-white d-flex justify-content-between align-items-center">
                                    <h6 class="mb-0">Edit Laundary Record</h6>
                                ...
                                <div class="card-body">
                                    <div class="row g-3">
                                        ...
                                        <div class="col-md-4">
                                            ...
                                            <input type="date" id="date" name="date" value="<?php echo h($val_date_db); ?>">
                                            <small id="dateError" class="text-danger d-none"></small>
                                        </div>

                                        <div class="col-md-4">
                                            <label class="form-label labelf">Gate Pass No.</label>
                                            <input type="text" name="gate_pass_no" id="gate_pass_no" maxlength="50"
                                                value="<?php echo h($val_gate); ?>" placeholder="Enter gate pass no">
                                            <small id="gatePassError" class="text-danger d-none"></small>
                                        </div>

                                        <div class="col-md-4">
                                            <label class="form-label labelf">Transport</label>
                                            <input type="text" name="transport_qty" id="transport_qty"
                                                value="<?php echo h($val_tq); ?>" placeholder="0.00">
                                            <small id="transportQtyError" class="text-danger d-none"></small>
                                        </div>

                                        <div class="col-md-4">
                                            <label class="form-label labelf">Uniforms Qty</label>
                                            <input type="text" name="uniforms_qty" id="uniforms_qty"
                                                value="<?php echo h($val_uq); ?>" placeholder="0.00">
                                            <small id="uniformsQtyError" class="text-danger d-none"></small>
                                        </div>

                                        <div class="col-md-4">
                                            <label class="form-label labelf">Caps Qty</label>
                                            <input type="text" name="caps_qty" id="caps_qty"
                                                value="<?php echo h($val_cq); ?>" placeholder="0.00">
                                            <small id="capsQtyError" class="text-danger d-none"></small>
                                        </div>

                                        <div class="col-md-4">
                                            <label class="form-label labelf">Herb Bags Qty</label>
                                            <input type="text" name="herb_bags_qty" id="herb_bags_qty"
                                                value="<?php echo h($val_hq); ?>" placeholder="0.00">
 ...
 -->
                                    ...
                                    <div class="text-center mt-4">
                                        <button type="submit" name="update" class="btn btn-form px-4" id="submitBtn">
                                            Update
                                        </button>

                                        <a href="laundary_record_list.php" class="btn btn-secondary btn-cancel">
                                            Cancel
                                        </a>
                                    </div>
                                </div>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            // ===== Client-side validation (same as laundary_form) =====
            const form = document.getElementById("laundaryEditForm");
            ...
            function validateDate() {
                ...
                if (!/^\d{4}-\d{2}-\d{2}$/.test(v)) {
                    setError(dateInput, dateError, "Invalid date.");
                    return false;
                }
            ...
            function validateGatePass() {
                ...
                if (!v) {
                    setError(gatePassNo, gatePassError, "Gate Pass No is required.");
                    return false;
                }
                if (v.length > 50) {
                    setError(gatePassNo, gatePassError, "Max 50 characters.");
                    return false;
                }
                clearError(gatePassNo, gatePassError);
                return true;
            }
            ...
            function validateNonNegativeNumber(input, errorElem, label) {
                ...
                if (!v) {
                    setError(input, errorElem, `${label} is required.`);
                    return false;
                }
                const num = Number(v);
                if (Number.isNaN(num) || num < 0) {
                    setError(input, errorElem, "Enter a valid non-negative number.");
                    return false;
                }
                ...
                try {
                    ...
                    if (data.ok) {
                        if (data.status === "updated") {
                            showToast("toast-success", data.msg || "Record updated successfully!");
                        } else {
                            showToast("toast-warning", data.msg || "No changes were made.");
                        }
                    } else {
                        if (data.status === "invalid") showToast("toast-warning", data.msg || "Please fill all fields correctly.");
                        else showToast("toast-error", data.msg || "Update failed!");
                    }
                } catch (err) {
