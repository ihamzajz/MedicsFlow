            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    <div class="row justify-content-center">
                        <div class="col-md-5">
                        <h3 class="text-center pb-3" style="font-size: 27px;">Edit Meters</h3>
                ...
                <?php
                    ...
                    ?>
                <form action="" method="POST" enctype="multipart/form-data" style="border: 2px solid black; padding:15px;border-radius:5px">
                    <div class="mb-3">
                        <label class="form-label" style="font-size: 15px">Name</label>
                        <input type="text" class="form-control" name="mname" required autocomplete="off" placeholder="<?php echo $row['name']; ?>">
                    </div>
                    <div class="mb-3">
                        <label class="form-label" style="font-size: 15px">Reading</label>
                        <input type="text" class="form-control" name="mreading" required autocomplete="off" placeholder="<?php echo $row['total_reading']; ?>">
                    </div>
                    <!-- <div class="mb-3">
                        <label class="form-label" style="font-size: 15px">Assign Department</label>
                        <input type="text" class="form-control" name="mdepartment" required autocomplete="off" placeholder="<?php echo $row['department_id']; ?>">
                    </div> -->
                    <div class="mb-3">
                        <label class="form-label" style="font-size: 15px">Assign Department</label>
                        <select class="form-select" name="mdepartment" required>
                            <option value="">Select Department</option>
                            <?php while ($department = mysqli_fetch_assoc($departments_result)) { ?>
                                <option value="<?php echo $department['id']; ?>" <?php if ($department['id'] == $row['department_id']) echo 'selected'; ?>>
                    ...
                    <!-- <div class="mb-3">
                        ...
                        <input type="text" class="form-control" name="muser" required autocomplete="off" placeholder="<?php echo $row['assigned_user_id']; ?>">
                    </div> -->
                    <div class="mb-3">
                <label class="form-label" style="font-size: 15px">Assign User</label>
                <select class="form-select" name="muser" required>
                    <option value="">Select User</option>
                    <?php while ($user = mysqli_fetch_assoc($users_result)) { ?>
                        <option value="<?php echo $user['id']; ?>" <?php if ($user['id'] == $row['assigned_user_id']) echo 'selected'; ?>>
                    ...
                    <div class="text-center">
                        <button type="submit" class="btn btn-primary w-25" name="meterUpdate">Update</button>
                    </div>
                </form>
                <?php
                    } else {
                        // Handle when no data is found for the given ID
                        echo "No data found for this ID.";
                    }
                    ?>
                <?php
                    if (isset($_POST['meterUpdate'])) {
                    $name =  $_POST['mname'];
                    $total_reading =  $_POST['mreading'];
                    $assign_department =  $_POST['mdepartment'];
                    $assign_user =  $_POST['muser'];							
                    
                               $update="UPDATE meters SET name='$name',total_reading ='$total_reading',department_id='$assign_department',assigned_user_id='$assign_user' WHERE id = '$id'";
