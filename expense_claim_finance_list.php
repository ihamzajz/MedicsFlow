...
/* =========================================================
   ✅ Finance Approve / Reject (SAME PAGE + saves finance_msg/finance_date)
   - NO message popup
   - PRG redirect to same URL to avoid resubmit
========================================================= */
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ecf_action'], $_POST['ecf_id'])) {
    $ecid = (int)($_POST['ecf_id'] ?? 0);
    $act  = (string)($_POST['ecf_action'] ?? '');
    ...
    if ($ecid > 0 && ($act === 'approve' || $act === 'reject')) {
        ...
        // finance_status, finance_msg, finance_date
        $update = "UPDATE expense_claim
                   SET finance_status = '{$status_safe}',
                       finance_msg    = '{$msg_safe}',
                       finance_date   = '{$now_safe}'
                   WHERE id = {$ecid}";
        mysqli_query($conn, $update);
    }

    // PRG redirect to same URL
...
<head>
    ...
    <title>Expense Claim - Finance Approval</title>
...
<body>
                ...
                <div class="container-xxl">
                    ...
                    <div class="card shadow-sm border-0 mb-3">
                        <div class="card-body d-flex align-items-center justify-content-between flex-wrap gap-2">
                            <div>
                                <h5 class="mb-0" style="font-weight:700;">Finance Approval</h5>
                            </div>
                                    ...
                                    <tbody>
                                        ...
                                        if ($data) {
                                            while ($row = mysqli_fetch_assoc($select_q)) {
                                                ...
                                                $financeStatusVal = trim((string)($row['finance_status'] ?? 'Pending'));
                                                if ($financeStatusVal === '') $financeStatusVal = 'Pending';
                                        ...
                                        ?>
                                                <tr id="row_<?php echo $rid; ?>">
                                                    <td style="white-space:nowrap;">
                                                        <a href="expense_claim_finance_details.php?id=<?php echo $rid; ?>" class="btn-details">
                                                            <i class="fa-solid fa-arrow-up me-1"></i> Details
                                                        </a>
                                                    </td>

                                                    <?php if (strcasecmp($financeStatusVal, 'Pending') === 0) { ?>
                                                        <td style="white-space:nowrap;">
                                                            <form method="POST" action="" class="m-0">
                                                                <input type="hidden" name="ecf_id" value="<?php echo $rid; ?>">
                                                                <input type="hidden" name="ecf_action" value="approve">
                                                        ...
                                                        <td style="white-space:nowrap;">
                                                            <form method="POST" action="" class="m-0">
                                                                <input type="hidden" name="ecf_id" value="<?php echo $rid; ?>">
                                                                <input type="hidden" name="ecf_action" value="reject">
                                                                <button type="submit" class="btn btn-danger btn-sm">
                                                                    <i class="fa-solid fa-xmark me-1"></i> Reject
                                                                </button>
                                                            </form>
                                                        </td>
                                                    <?php } else if (strcasecmp($financeStatusVal, 'Approved') === 0) { ?>
                                                        <td style="white-space:nowrap;">
                                                            <span class="text-success fw-bold">Approved</span>
                                                        </td>
                                                        <td></td>
                                                    <?php } else if (strcasecmp($financeStatusVal, 'Rejected') === 0) { ?>
                                                        <td style="white-space:nowrap;">
                                                            <span class="text-danger fw-bold">Rejected</span>
                                                        </td>
                                                        <td></td>
                                                    <?php } else { ?>
                                                        <td style="white-space:nowrap;"><?php echo htmlspecialchars($financeStatusVal); ?></td>
                                                        <td></td>
                                                    <?php } ?>
    ...
    <script>
        document.getElementById('exportBtn').addEventListener('click', function() {
            ...
            XLSX.utils.book_append_sheet(wb, ws, 'Finance Approval');

            XLSX.writeFile(wb, 'expense_claim_finance_list.xlsx');
        });
    </script>
