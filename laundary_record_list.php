...
$hasAnyFilter = ($month!=='' || $year!=='' || $date_from!=='' || $date_to!=='' || $search!=='' || $editId>0);
if (!$errors && $isApply && !($month!=='' || $year!=='' || $date_from!=='' || $date_to!=='' || $search!=='')) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_record'])) {
    ...
    $pdate = trim($_POST['date'] ?? '');
    $pgate = trim($_POST['gate_pass_no'] ?? '');
    $ptransport = trim($_POST['transport_qty'] ?? '0');
    $puniforms  = trim($_POST['uniforms_qty'] ?? '0');
    $pcaps      = trim($_POST['caps_qty'] ?? '0');
    $pherb      = trim($_POST['herb_bags_qty'] ?? '0');
    $puser      = trim($_POST['form_user'] ?? '');

    if ($pid <= 0) $errors[] = "Invalid record id.";
    if (!isValidDateYmd($pdate)) $errors[] = "Invalid date.";
    if ($pgate === '') $errors[] = "Gate Pass No. is required.";
    if ($puser === '') $errors[] = "User is required.";

    // numeric normalize
    $ptransport = is_numeric($ptransport) ? (float)$ptransport : 0.0;
    $puniforms  = is_numeric($puniforms)  ? (float)$puniforms  : 0.0;
    $pcaps      = is_numeric($pcaps)      ? (float)$pcaps      : 0.0;
    $pherb      = is_numeric($pherb)      ? (float)$pherb      : 0.0;

    if (!$errors) {
        $sqlU = "UPDATE laundary_form
                 SET `date`=?, gate_pass_no=?, transport_qty=?, uniforms_qty=?, caps_qty=?, herb_bags_qty=?, form_user=?
                 WHERE id=?";
        $stmtU = $conn->prepare($sqlU);
        $stmtU->bind_param(
            "ssddddsi",
            $pdate, $pgate,
            $ptransport, $puniforms, $pcaps, $pherb,
            $puser, $pid
        );
        $stmtU->execute();
        $stmtU->close();

        // redirect back to keep filters/page but remove id
        $qs = buildQuery(['id'=>null]);
        header("Location: laundary_record_list.php" . ($qs ? "?".$qs : ""));
        exit;
    } else {
        // keep edit mode visible
        $editId = $pid;
    }
}
...
/* ----------------- Load edit record (if id) ----------------- */
if (!$errors && $editId > 0) {
    $stmtE = $conn->prepare("SELECT id, `date`, gate_pass_no, transport_qty, uniforms_qty, caps_qty, herb_bags_qty, form_user
                             FROM laundary_form WHERE id=?");
    $stmtE->bind_param("i", $editId);
    $stmtE->execute();
    $resE = $stmtE->get_result();
    $editRow = $resE->fetch_assoc() ?: null;
    $stmtE->close();
...
$hasFilter = ($month!=='' || $year!=='' || $date_from!=='' || $date_to!=='' || $search!=='');
?>
<!doctype html>
<html lang="en">
<head>
    ...
    <title>Laundary Record List</title>
...
.btn-edit,.btn-edit:hover{ background:var(--edit); border:1px solid var(--edit); color:#fff; }
 ...
 .btn-edit{ background:var(--edit); border:1px solid var(--edit); color:#fff; }
        ...
        .edit-form .form-control{ height:36px; border-radius:10px; }
        .edit-form .form-select{ height:36px; border-radius:10px; }
    </style>
...
<div class="wrapper">
                ...
                <div class="col-12">
                    ...
                    <div class="dash-card">
                        ...
                        <div class="p-3 p-md-4">
                            ...
                            <!-- EDIT FORM (only when id is present) -->
                            <?php if ($editRow): ?>
                                <div class="panel panel-pad mb-3 edit-form">
                                    <div class="d-flex justify-content-between align-items-center flex-wrap gap-2 mb-2">
                                        <span class="badge-soft"><i class="fa-solid fa-pen-to-square"></i> Edit Record (Ref# <?php echo (int)$editRow['id']; ?>)</span>
                                        <a class="btn btn-sm btn-sq btn-clear" href="?<?php echo h(buildQuery(['id'=>null])); ?>">
                                    ...
                                    <form method="POST">
                                        ...
                                        <div class="row g-2">
                                            <div class="col-12 col-md-2">
                                                <label>Date</label>
                                                <input type="date" class="form-control" name="date" value="<?php echo h($editRow['date']); ?>" required>
                                            </div>

                                            <div class="col-12 col-md-2">
                                                <label>Gate Pass No.</label>
                                                <input type="text" class="form-control" name="gate_pass_no" value="<?php echo h($editRow['gate_pass_no']); ?>" required>
                                            </div>

                                            <div class="col-12 col-md-2">
                                                <label>Transport</label>
                                                <input type="number" step="0.01" class="form-control" name="transport_qty" value="<?php echo h($editRow['transport_qty']); ?>">
                                            </div>

                                            <div class="col-12 col-md-2">
                                                <label>Uniforms Qty</label>
                                                <input type="number" step="0.01" class="form-control" name="uniforms_qty" value="<?php echo h($editRow['uniforms_qty']); ?>">
                                            </div>

                                            <div class="col-12 col-md-2">
                                                <label>Caps Qty</label>
                                                <input type="number" step="0.01" class="form-control" name="caps_qty" value="<?php echo h($editRow['caps_qty']); ?>">
                                            </div>

                                            <div class="col-12 col-md-2">
                                                <label>Herb Bags Qty</label>
                                                <input type="number" step="0.01" class="form-control" name="herb_bags_qty" value="<?php echo h($editRow['herb_bags_qty']); ?>">
                                            </div>

                                            <div class="col-12 col-md-3">
                                                <label>User</label>
                                                <input type="text" class="form-control" name="form_user" value="<?php echo h($editRow['form_user']); ?>" required>
                                            </div>
                            ...
                            <div class="panel panel-pad">
                                ...
                                <form method="GET">
                                    ...
                                    <div class="filter-actions">
                                        ...
                                        <div class="right">
                                            <?php if ($hasFilter || $editId>0): ?>
                                                <a class="btn btn-sm btn-sq btn-clear" href="laundary_record_list.php">
                                ...
                                <div class="table-wrap">
                                    <div class="table-responsive">
                                        <table class="table table-hover align-middle">
                                            ...
                                            <tbody>
                                            <?php if ($errors): ?>
                                                <tr><td colspan="9" class="text-center text-muted py-4">Fix filter errors and click Apply.</td></tr>
                                            <?php elseif (!$rows): ?>
   ...
   href="laundary_record_edit.php?id=<?php echo (int)$r['id']; ?>">
   <i class="fa-solid fa-pen"></i> Edit
                                            ...
                                            <tfoot>
                                                <tr class="calc-sep"><td colspan="9"></td></tr>
