...
/* =========================================================
   ✅ IT Approval (Pending only) - SAME LOGIC (UNCHANGED)
   ✅ Server-side pagination (50)
   ✅ Global search across ALL pages
   ✅ Department filter
   ✅ Date From/To (both required, To >= From) + error msg
   ✅ Reset
   ✅ Total records badge (NOT pagination visible count)
   ✅ Date shown dd-mm-yyyy (db datetime)
   ✅ Keep approve/reject button logic (reject prompt)
========================================================= */
...
/* Date range validation + filter (db has datetime -> DATE(date)) */
$hasFrom = ($dateFrom !== '');
$hasTo   = ($dateTo !== '');
...
<head>
  ...
  <title>IT Accessories - IT Approval</title>
        ...
        <div class="container-xxl">
          ...
          <div class="card shadow-sm border-0 mb-3">
            <div class="card-body d-flex align-items-center justify-content-between flex-wrap gap-2">
              <div>
                <h5 class="mb-0 box1-heading">IT Accessories - IT Approval (Pending)</h5>
              </div>
          ...
          <div class="card shadow-sm border-0 mb-3">
            <div class="card-body">
              <form id="searchForm" method="GET" action="" class="row g-2 align-items-end">
                ...
                <div class="col-12 col-lg-12 d-flex justify-content-lg-end align-items-end mt-1">
                  ...
                  <tbody>
                    ...
                    if ($data) {
                      while ($row = mysqli_fetch_assoc($select_q)) {
                        ...
                        $dateOnly = '';
                        if (!empty($row['date'])) {
                          try { $dateOnly = (new DateTime($row['date']))->format('d-m-Y'); }
                          catch (Exception $e) { $dateOnly = ''; }
                        }
                    ?>
                        <tr id="row_<?php echo $rid; ?>">
                          ...
                          <td style="white-space:nowrap;">
                            <a
                              href="itacc_approve.php?id=<?php echo $rid; ?>&email=<?php echo urlencode($rowEmail); ?>&name=<?php echo urlencode($rowName); ?>"
                              class="btn-approve">
                          ...
                          <td class="td-wrap"><?php echo htmlspecialchars((string)($row['email'] ?? '')); ?></td>
    ...
    <script>
      ...
      function promptReason(itemId) {
        ...
        if (reason != null && reason.trim() !== "") {
          window.location.href = "itacc_reject.php?id=" + itemId + "&reason=" + encodeURIComponent(reason.trim());
        }
      }
    </script>
