...
// ✅ Head dept session (your old logic)
$be_depart   = $_SESSION['be_depart'] ?? '';
$head_depart = $_SESSION['be_depart'] ?? ''; // same as your old code

/* =========================================================
   ✅ SAME LOGIC (UNCHANGED) - Head Approval
========================================================= */
$perPage = 50;
...
// ✅ Base WHERE (KEEP your same data conditions)
if ($be_depart === 'super') {
  $whereParts[] = "1=1";
} else {
  $head_depart_safe = mysqli_real_escape_string($conn, (string)$head_depart);

  // ✅ exactly your old condition
  $whereParts[] = "(
      be_depart = '{$head_depart_safe}' AND
      be_role = 'user' AND
      head_status = 'Pending' AND
      (finance_status = 'Pending' OR admin_status = 'Pending')
  )";
}

// ✅ Global Search (same pattern as engineering page)
if ($qRaw !== '') {
  ...
  $whereParts[] = "(
      ...
      OR depart_type LIKE '{$like}'
      OR description LIKE '{$like}'
      OR email LIKE '{$like}'
  )";
}

// ✅ Date range validation + filter (db has datetime)
$hasFrom = ($dateFrom !== '');
$hasTo   = ($dateTo !== '');
...
<head>
  ...
  <title>Work Order - Head Approval</title>
  ...
  <style>
    ...
    /* ✅ Action buttons (same as engineering) */
    .btn-details {
      font-size: 11px;
        ...
        <div class="container-xxl">
          ...
          <div class="card shadow-sm border-0 mb-3">
            <div class="card-body d-flex align-items-center justify-content-between flex-wrap gap-2">
              <div>
                <h5 class="mb-0 box1-heading">Head Approval</h5>
              </div>
                  ...
                  <tbody>
                    ...
                    if ($data) {
                      while ($row = mysqli_fetch_assoc($select_q)) {
                        ...
                        $rowDepartType = (string)($row['depart_type'] ?? '');
                    ...
                    ?>
                        <tr id="row_<?php echo $rid; ?>">
                          ...
                          <td style="white-space:nowrap;">
                            <a href="workorder_head_details.php?id=<?php echo $rid; ?>" class="btn-details">
                          ...
                          <td style="white-space:nowrap;">
                            <a
                              href="workorder_head_approve.php?id=<?php echo $rid; ?>&email=<?php echo urlencode($rowEmail); ?>&name=<?php echo urlencode($rowName); ?>&depart_type=<?php echo urlencode($rowDepartType); ?>&type=<?php echo urlencode($rowType); ?>&category=<?php echo urlencode($rowCategory); ?>"
                              class="btn-approve">
                          ...
                          <td style="white-space:nowrap;">
                            <a
                              href="workorder_head_reject.php?id=<?php echo $rid; ?>&email=<?php echo urlencode($rowEmail); ?>&name=<?php echo urlencode($rowName); ?>"
                              class="btn-reject">
