...
if (!workorder_can_admin_act()) {
    workorder_abort(403, 'You are not allowed to reject admin workorders.');
}
...
if (strcasecmp((string)($request['depart_type'] ?? ''), 'Admin') !== 0 || !workorder_request_is_admin_pending($request)) {
    workorder_flash('danger', 'This request is no longer available for admin rejection.');
    workorder_redirect('workorder_admin_list.php');
}
...
$stmt = workorder_prepare(
    'UPDATE workorder_form
     SET admin_status = ?, admin_msg = ?, admin_date = ?, task_status = ?, final_status = ?, reason = ?
     WHERE id = ? AND admin_status = ?'
);
$stmt->bind_param('ssssssis', $rejected, $adminMsg, $now, $taskStatus, $finalStatus, $reason, $requestId, $pending);
$stmt->execute();
...
workorder_log_action($requestId, 'admin', 'rejected', $reason);
...
try {
    ...
    <p><strong>Reason:</strong> " . nl2br(workorder_h($reason)) . "</p>
    ...
    error_log('Workorder admin reject mail failed: ' . $e->getMessage());
