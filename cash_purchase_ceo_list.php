...
/* =========================================================
   ✅ CEO Approve / Reject (SAME PAGE + saves ceo_msg/ceo_date)
   - NO message
   - PRG redirect to same URL to avoid resubmit
   - Updates ceo_status column (and ceo_msg/ceo_date)
========================================================= */
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['cp_action'], $_POST['cp_id'])) {
    ...
    if ($cpid > 0 && ($act === 'approve' || $act === 'reject')) {
        ...
        // ✅ CEO columns in cash_purchase table
        $update = "UPDATE cash_purchase
               SET ceo_status = '{$status_safe}',
                   ceo_msg    = '{$msg_safe}',
                   ceo_date   = '{$now_safe}'
               WHERE id = {$cpid}";
...
<head>
    ...
    <title>Cash Purchase - CEO Approval</title>
                                        ...
                                        ?>
                                                <tr id="row_<?php echo $rid; ?>">
                                                    <td style="white-space:nowrap;">
                                                        <a href="cash_purchase_ceo_details.php?id=<?php echo $rid; ?>" class="btn-details">
    ...
    <script>
        document.getElementById('exportBtn').addEventListener('click', function() {
            ...
            $('#myTable tbody tr:visible').each(function() {
                ...
                // indexes are stable because we keep 2 action cols always
                data.push([
            ...
            XLSX.writeFile(wb, 'ceo_approval_list.xlsx');
        });
    </script>
