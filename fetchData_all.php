...
$query = "SELECT * FROM raheel1 WHERE 1=1";

if ($search) {
    $query .= " AND (
        dp LIKE '%$search%' OR 
        depot_name LIKE '%$search%' OR
        cust LIKE '%$search%' OR 
        customer_name LIKE '%$search%' OR 
        address LIKE '%$search%' OR 
        class LIKE '%$search%' OR 
        inv LIKE '%$search%' OR 
        ivc_date LIKE '%$search%' OR
        prd LIKE '%$search%' OR
        pack LIKE '%$search%' OR 
        batch LIKE '%$search%' OR
        sales_value LIKE '%$search%' OR 
        tp_medics LIKE '%$search%' OR
        cp_medics LIKE '%$search%' OR 
        cp_value LIKE '%$search%' OR 
        deff LIKE '%$search%' OR 
        product_name LIKE '%$search%' OR 
        sales_qty LIKE '%$search%' OR 
        bounty_qty LIKE '%$search%')";
}
...
$total_query = "SELECT COUNT(*) as total FROM raheel1 WHERE 1=1";
