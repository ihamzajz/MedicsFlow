...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax']) && $_POST['ajax'] === '1') {
    // IMPORTANT: return only JSON (no HTML / warnings)
    ...
    // helper: validate int >= 0
    function valid_nonneg_int($v) {
        if ($v === '' || !is_numeric($v)) return false;
        if ((int)$v < 0) return false;
        // prevent decimals like 2.5
        if ((string)(int)$v !== (string)(0 + $v)) return false;
        return true;
    }

    $ints = [
        $no_of_employee, $no_of_guest,
        $total_head_counts_hr, $security_guards_part2, $guests_part2,
        $attendance, $security_guards_part3, $guests_part3
    ];
    foreach ($ints as $v) {
        if (!valid_nonneg_int($v)) { $ok = false; break; }
    }
    ...
    if ($ok) {
        // ===== Compute totals/diffs on server (don’t trust client totals) =====
        ...
        if ($stmt) {
            // DECIMAL(7,2) => bind as double (d)
            $dp2 = round((float)$diff_percent_part2, 2);
            $dp3 = round((float)$diff_percent_part3, 2);

            // MUST match 20 params exactly
            // ss + 8i + d + 5i + d + sss  = 20
            $stmt->bind_param(
                "ssiiiiiiiidiiiiidsss",
                $date, $day,
                $no_of_employee, $no_of_guest, $total_part1,
                $total_head_counts_hr, $security_guards_part2, $guests_part2, $total_part2, $diff_part2, $dp2,
                $attendance, $security_guards_part3, $guests_part3, $total_part3, $diff_part3, $dp3,
                $remarks, $form_user, $created_at
            );
            ...
            if ($exec) {
                echo json_encode(['ok'=>true,'msg'=>'Lunch form has been submitted!']);
                exit;
            } else {
...
// --- For GET render ---
$today_db = date('Y-m-d'); // yyyy-mm-dd
$today_day = date('l');
?>
<!DOCTYPE html>
<html>
                            ...
                            <div class="card-body">
                                ...
                                <div class="section-box">
                                    ...
                                    <div class="row g-3">
                                        ...
                                        <div class="col-md-2">
                                            ...
                                            <input type="text" id="total_part1" value="0" readonly>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="section-box">
                                    ...
                                    <div class="row g-3">
                                        ...
                                        <div class="col-md-2">
                                            <label class="form-label labelf">Total</label>
                                            <input type="text" id="total_part2" value="0" readonly>
                                        </div>

                                        <div class="col-md-2">
                                            <label class="form-label labelf">Diff / Diff%</label>
                                            <input type="text" id="diff_part2" value="0 (0.00%)" readonly>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="section-box">
                                    ...
                                    <div class="row g-3">
                                        ...
                                        <div class="col-md-2">
                                            <label class="form-label labelf">Total</label>
                                            <input type="text" id="total_part3" value="0" readonly>
                                        </div>

                                        <div class="col-md-2">
                                            <label class="form-label labelf">Diff / Diff%</label>
                                            <input type="text" id="diff_part3" value="0 (0.00%)" readonly>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="row g-3">
                                    <div class="col-md-12">
                                        ...
                                        <textarea class="form-control" name="remarks" id="remarks"
                                            rows="2" minlength="1" maxlength="500"
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    function validateDate(){
        ...
        if (!v) { setError(dateEl, dateError, "Date is required."); return false; }
        clearError(dateEl, dateError); return true;
    }

    function validateIntField(el, errEl, label){
        const v = el.value.trim();
        if (v === "") { setError(el, errEl, label + " is required."); return false; }
        const n = Number(v);
        if (Number.isNaN(n) || n < 0 || !Number.isInteger(n)) { setError(el, errEl, "Enter valid non-negative integer."); return false; }
    ...
    function validateRemarks(){
        ...
        if (v.length > 500) { setError(remarksEl, remarksError, "Max 500 characters."); return false; }
        clearError(remarksEl, remarksError); return true;
    }

    // listeners
    dateEl.addEventListener("change", function(){ updateDay(); });
    ...
    form.addEventListener("submit", async function(e) {
        ...
        try {
            ...
            if (data.ok) {
                showToast("toast-success", data.msg || "Lunch form has been submitted!");

                // reset fields (keep date = today)
                ...
                [empEl, guestEl, hrEl, sec2El, g2El, attEl, sec3El, g3El].forEach(el => el.value = "");
                remarksEl.value = "";

                // clear errors
                form.querySelectorAll(".is-invalid").forEach(el => el.classList.remove("is-invalid"));
                [dateError, empError, guestError, hrError, sec2Error, g2Error, attError, sec3Error, g3Error, remarksError]
