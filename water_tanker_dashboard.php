 ...
 * Filters stay unchanged because DB/query still uses Y-m-d.
 */
...
if (!$errors) {
    ...
    // Search
    if ($search !== '') {
        $where[] = "(slip_no LIKE ? OR deliver_at LIKE ? OR reason_for LIKE ? OR form_user LIKE ?)";
        $types .= "ssss";
        $like = "%{$search}%";
        $params[] = $like;
        $params[] = $like;
...
function fetchAllFilteredRows($conn, $whereSql, $types, $params) {
    $sql = "
        SELECT id, `date`, slip_no, deliver_at, amount, reason_for, form_user
        FROM water_tanker_form
        {$whereSql}
...
$reportTitle = "Water Tanker Report" . ($parts ? " (" . implode(", ", $parts) . ")" : "");
...
if (!$errors && $export === 'excel') {
    ...
    $filename = "water_tanker_report_" . date("Y-m-d_H-i") . ".csv";
    ...
    // Header
    fputcsv($out, ["Ref#", "Date", "Slip No", "Deliver At", "Amount", "Reason", "User"]);

    foreach ($exportRows as $r) {
        fputcsv($out, [
            (int)$r['id'],
            formatDateDMY((string)$r['date']), // ✅ UI format in Excel too (optional, safe)
            (string)$r['slip_no'],
            (string)$r['deliver_at'],
...
if (!$errors) {
    // Count
    $stmtC = $conn->prepare("SELECT COUNT(*) FROM water_tanker_form {$whereSql}");
...
<head>
    ...
    <title>Water Tanker Dashboard</title>
                ...
                <div class="col-12 col-xl-11">
                    ...
                    <div class="dash-card">
                        <div class="dash-header">
                            <h6>Water Tanker Dashboard</h6>
                            <a href="water_tanker_home.php" class="btn btn-light btn-sm btn-sq">
                            ...
                            <div class="panel panel-pad">
                                ...
                                <form method="GET">
                                    ...
                                    <div class="row g-2 align-items-end">
                                        ...
                                        <div class="col-12 col-md-3">
                                            ...
                                            <input type="text" class="form-control" name="search"
                                                   placeholder="Slip, deliver, reason, user..."
                                    ...
                                    <div class="filter-actions">
                                        ...
                                        <div class="right">
                                            <?php if ($hasFilter): ?>
                                                <a class="btn btn-sm btn-sq btn-clear" href="water_tanker_dashboard.php">
                                    ...
                                    <div class="table-responsive">
                                        <table class="table table-hover align-middle">
                                            ...
                                            <tr>
                                                ...
                                                <th style="width:140px;">Slip No</th>
                                                <th style="width:220px;">Deliver At</th>
                                            ...
                                            <tbody>
                                            <?php if ($errors): ?>
                                                <tr><td colspan="7" class="text-center text-muted py-4">Fix filter errors and click Apply.</td></tr>
                                            <?php elseif (!$rows): ?>
                                            ...
                                            <?php else: ?>
                                                <?php foreach ($rows as $r): ?>
                                                    <tr>
                                                        ...
                                                        <!-- ✅ ONLY CHANGE: display date as dd-mm-yyyy -->
                                                        <td><?php echo h(formatDateDMY($r['date'])); ?></td>
                                                        <td><?php echo h($r['slip_no']); ?></td>
                                                        <td><?php echo h($r['deliver_at']); ?></td>
                                                        <td class="text-end amount"><?php echo number_format((float)$r['amount'], 2); ?></td>
                                                        <td><?php echo h($r['reason_for']); ?></td>
                                            ...
                                            <tfoot>
                                            <tr>
                                                <td colspan="4" class="text-end fw-bold" style="font-size:12px;">Total Amount</td>
                                                <td class="text-end fw-bold" style="font-size:12px;">
                                                    <?php echo number_format((float)$sumPage, 2); ?>
                                                </td>
                                                <td colspan="2"></td>
                                            </tr>
                                            </tfoot>
...
/**
 * ✅ ONLY CHANGE for PDF:
 * Convert date column (index 1) to dd-mm-yyyy.
 * Filtering stays on Y-m-d in PHP queries, we only change display value in export rows.
 */
const EXPORT_ROWS = <?php echo json_encode(array_map(function($r){
    return [
        (int)$r['id'],
        formatDateDMY((string)$r['date']), // ✅ dd-mm-yyyy in PDF
        (string)$r['slip_no'],
        (string)$r['deliver_at'],
        number_format((float)$r['amount'], 2, '.', ''),
        (string)$r['reason_for'],
...
function downloadPDF() {
    ...
    doc.autoTable({
        startY: 20,
        head: [[ "Ref#", "Date", "Slip No", "Deliver At", "Amount", "Reason", "User" ]],
        body: EXPORT_ROWS,
        styles: { fontSize: 8 },
    ...
    doc.save("water_tanker_report.pdf");
}
</script>
