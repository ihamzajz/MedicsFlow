...
if (!workorder_can_admin_act()) {
  workorder_abort(403, 'You are not allowed to access admin cost evaluation.');
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['request_id'])) {
  ...
  if ($reqid > 0 && isset($_POST[$btnKey])) {
    ...
    if ($budget === '' || !is_numeric($budget) || (float)$budget < 0) {
      ...
      workorder_redirect('workorder_admin_estimatecost.php');
    }

    $request = workorder_fetch_request($reqid);
    if (!$request || strcasecmp((string)($request['depart_type'] ?? ''), 'Admin') !== 0 || (string)($request['amount'] ?? '') !== 'TBD') {
      workorder_flash('danger', 'This request is no longer available for cost evaluation.');
      workorder_redirect('workorder_admin_estimatecost.php');
    }

    $stmt = workorder_prepare("UPDATE workorder_form SET amount = ? WHERE id = ? AND depart_type = 'Admin' AND amount = 'TBD'");
    ...
    workorder_log_action($reqid, 'admin', 'estimated', 'Estimated amount set to ' . $budget);
    ...
    try {
      ...
      error_log('Workorder admin estimate mail failed: ' . $e->getMessage());
      $mailWarning = true;
    }
...
/* =========================================================
   ✅ Server-side Pagination + Search + Date Range (SAME UI/LOGIC)
   ========================================================= */
$perPage = 50;
...
<head>
  ...
  <title>Work Order - Cost Evaluation (Admin)</title>
  ...
  <style>
    ...
    /* =========================================================
       ✅ SAME TABLE UX AS ENGINEERING:
       1) TABLE HEIGHT = 90vh (scroll inside table)
       2) NO MORE "Urgent" going next line
        ...
        <div class="container-xxl">
          ...
          <div class="card shadow-sm border-0 mb-3">
            <div class="card-body d-flex align-items-center justify-content-between flex-wrap gap-2">
              <div>
                <h5 class="mb-0 box1-heading">Admin - Cost EvaluatioN</h5>
              </div>
                    ...
                    ?>
                        <tr id="row_<?php echo $rid; ?>">
                          ...
                          <td style="white-space:nowrap;">
                            <!-- ✅ Admin sends to Engineering (same as your old logic) -->
                            <a href="sendtoengineering.php?id=<?php echo $rid; ?>" class="btn-details">
                              <i class="fa-solid fa-infinity"></i> Engineering
                            </a>
                          </td>
