...
/* =========================================================
   ✅ CEO Approve / Reject (SAME PAGE + saves ceo_msg/ceo_date + closes status)
   - NO message popup
   - PRG redirect to same URL to avoid resubmit
   - Only allows action if finance_status = Approved (optional safeguard)
========================================================= */
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ecc_action'], $_POST['ecc_id'])) {
    $ecid = (int)($_POST['ecc_id'] ?? 0);
    $act  = (string)($_POST['ecc_action'] ?? '');

    if ($ecid > 0 && ($act === 'approve' || $act === 'reject')) {
        $approver_name = $_SESSION['fullname'] ?? '';
        $now = date('Y-m-d H:i:s');

        if ($act === 'approve') {
            $newCeoStatus = 'Approved';
            $msg = "Approved By $approver_name";
            $finalStatus = 'Closed'; // ✅ CEO approve closes request
        } else {
            $newCeoStatus = 'Rejected';
            $msg = "Rejected By $approver_name";
            $finalStatus = 'Open';   // ✅ keep open (you can change if you want closed)
        }

        $ceo_safe   = mysqli_real_escape_string($conn, $newCeoStatus);
        $msg_safe   = mysqli_real_escape_string($conn, $msg);
        $now_safe   = mysqli_real_escape_string($conn, $now);
        $final_safe = mysqli_real_escape_string($conn, $finalStatus);

        // ✅ assuming these columns exist in expense_claim table:
        // ceo_status, ceo_msg, ceo_date, status
        // Optional safeguard: only finance approved goes to CEO:
        $update = "UPDATE expense_claim
                   SET ceo_status = '{$ceo_safe}',
                       ceo_msg    = '{$msg_safe}',
                       ceo_date   = '{$now_safe}',
                       status     = '{$final_safe}'
                   WHERE id = {$ecid}";
...
/* =========================================================
   ...
   ✅ SAME UI as cash_purchase_admin_list
========================================================= */
$perPage = 50;
...
$ceoFilter     = isset($_GET['ceoStatus']) ? trim((string)$_GET['ceoStatus']) : 'All'; // ✅ NEW
...
/* CEO status filter */
$validCR = ['All', 'Pending', 'Approved', 'Rejected'];
if (!in_array($ceoFilter, $validCR, true)) $ceoFilter = 'All';
if ($ceoFilter !== 'All') {
    $c = mysqli_real_escape_string($conn, $ceoFilter);
    $whereParts[] = "ceo_status = '{$c}'";
}
...
/* Count */
$countSql = "SELECT COUNT(*) AS total FROM expense_claim WHERE {$whereSql}";
...
/* Fetch page */
$select = "SELECT * FROM expense_claim
           WHERE {$whereSql}
...
<head>
    ...
    <title>Expense Claim - CEO Approval</title>
                ...
                <div class="container-xxl">
                    ...
                    <div class="card shadow-sm border-0 mb-3">
                        <div class="card-body d-flex align-items-center justify-content-between flex-wrap gap-2">
                            <div>
                                <h5 class="mb-0" style="font-weight:700;">CEO Approval</h5>
                            </div>
                            <div class="d-flex gap-2 align-items-center flex-wrap">
                                <a class="btn btn-dark btn-sm" href="expense_claim_home.php">
                    ...
                    <div class="card shadow-sm border-0 mb-3">
                        <div class="card-body">
                            <form id="searchForm" method="GET" action="" class="row g-2 align-items-end">
                                ...
                                <div class="col-6 col-lg-2">
                                    <label class="form-label small mb-1">CEO Status</label>
                                    <select class="form-select form-select-sm" id="ceoStatus" name="ceoStatus">
                                        <option value="All" <?php echo ($ceoFilter === 'All' ? 'selected' : ''); ?>>All</option>
                                        <option value="Pending" <?php echo ($ceoFilter === 'Pending' ? 'selected' : ''); ?>>Pending</option>
                                        <option value="Approved" <?php echo ($ceoFilter === 'Approved' ? 'selected' : ''); ?>>Approved</option>
                                        <option value="Rejected" <?php echo ($ceoFilter === 'Rejected' ? 'selected' : ''); ?>>Rejected</option>
                                    </select>
                                </div>
                                    ...
                                    <tbody>
                                        ...
                                        if ($data) {
                                            while ($row = mysqli_fetch_assoc($select_q)) {
                                                ...
                                                $ceoStatusVal = trim((string)($row['ceo_status'] ?? 'Pending'));
                                                if ($ceoStatusVal === '') $ceoStatusVal = 'Pending';
                                        ...
                                        ?>
                                                <tr id="row_<?php echo $rid; ?>">
                                                    <td style="white-space:nowrap;">
                                                        <a href="expense_claim_ceo_details.php?id=<?php echo $rid; ?>" class="btn-details">
                                                            <i class="fa-solid fa-arrow-up me-1"></i> Details
                                                        </a>
                                                    </td>

                                                    <?php if (strcasecmp($ceoStatusVal, 'Pending') === 0) { ?>
                                                        <td style="white-space:nowrap;">
                                                            <form method="POST" action="" class="m-0">
                                                                <input type="hidden" name="ecc_id" value="<?php echo $rid; ?>">
                                                                <input type="hidden" name="ecc_action" value="approve">
                                                        ...
                                                        <td style="white-space:nowrap;">
                                                            <form method="POST" action="" class="m-0">
                                                                <input type="hidden" name="ecc_id" value="<?php echo $rid; ?>">
                                                                <input type="hidden" name="ecc_action" value="reject">
                                                                <button type="submit" class="btn btn-danger btn-sm">
                                                                    <i class="fa-solid fa-xmark me-1"></i> Reject
                                                                </button>
                                                            </form>
                                                        </td>
                                                    <?php } else if (strcasecmp($ceoStatusVal, 'Approved') === 0) { ?>
                                                        <td style="white-space:nowrap;">
                                                            <span class="text-success fw-bold">Approved</span>
                                                        </td>
                                                        <td></td>
                                                    <?php } else if (strcasecmp($ceoStatusVal, 'Rejected') === 0) { ?>
                                                        <td style="white-space:nowrap;">
                                                            <span class="text-danger fw-bold">Rejected</span>
                                                        </td>
                                                        <td></td>
                                                    <?php } else { ?>
                                                        <td style="white-space:nowrap;"><?php echo htmlspecialchars($ceoStatusVal); ?></td>
                                                        <td></td>
                                                    <?php } ?>
                                                    ...
                                                    <td><?php echo htmlspecialchars((string)($row['amount'] ?? '0')); ?></td>
                                                    <td class="td-wrap"><?php echo htmlspecialchars((string)($row['reason_for_claim'] ?? '')); ?></td>
                                                    <td><?php echo htmlspecialchars((string)($row['admin_status'] ?? '')); ?></td>
                                                    <td><?php echo htmlspecialchars((string)($row['finance_status'] ?? '')); ?></td>
                                                    <td><?php echo htmlspecialchars((string)($row['ceo_status'] ?? '')); ?></td>
                                                    <td><?php echo htmlspecialchars((string)($row['status'] ?? '')); ?></td>
                                                </tr>
                                        <?php
                                            }
                                        } else {
                                            echo "<tr><td colspan='13' class='text-muted fw-semibold p-3'>No record found!</td></tr>";
                                        }
                                        ?>
                                    </tbody>
                                </table>
        ...
        document.getElementById('exportBtn').addEventListener('click', function() {
            ...
            $('#myTable tbody tr:visible').each(function() {
                ...
                data.push([
                    ...
                    cols.eq(7).text().trim(),  // Amount
                    cols.eq(8).text().trim(),  // Reason
                    cols.eq(9).text().trim(),  // Admin Status
                    cols.eq(10).text().trim(), // Finance Status
                    cols.eq(11).text().trim(), // CEO Status
                    cols.eq(12).text().trim()  // Final Status
                ]);
            });
            ...
            XLSX.utils.book_append_sheet(wb, ws, 'CEO Approval');

            XLSX.writeFile(wb, 'expense_claim_ceo_list.xlsx');
        });
    </script>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            form.addEventListener("submit", function(e) {
                ...
                const c = (document.getElementById("ceoStatus").value || "All").trim();
                ...
                if (c === "All") url.searchParams.delete("ceoStatus");
                else url.searchParams.set("ceoStatus", c);
            ...
            resetBtn.addEventListener("click", function() {
                ...
                url.searchParams.delete("ceoStatus");
