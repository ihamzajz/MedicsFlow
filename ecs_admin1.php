   ...
   <body>
      ...
      <div class="wrapper d-flex align-items-stretch">
         ...
         <div id="content">
            ...
            <h3 class="text-center pb-3" style="font-size: 27px;">Administrator 1</h3>
            ...
            <button id="excel" class="btn btn-success btn-sm dataExport" data-type="excel">Excel</button>	<?php
               ...
               JOIN users ON meters.assigned_user_id = users.id
                WHERE department.department_name = 'Finance' OR department.department_name = 'Information Technology'";
         ...
         //           users.fullname,
         //           mr.last_reading_value
         //       FROM meters
         //       JOIN department ON meters.department_id = department.id
         //       JOIN users ON meters.assigned_user_id = users.id
         //       LEFT JOIN (
         //           SELECT meter_id, MAX(reading_date) AS last_reading_date, reading_value AS last_reading_value
         //           FROM meter_reading
         //           GROUP BY meter_id
         //       ) mr ON meters.id = mr.meter_id
         //       WHERE department.department_name = 'Finance' OR department.department_name = 'Information Technology'
         //   ";
            ...
            <table  class="table table-bordered mt-1" id="myTable">
               ...
               <tbody>
                  ...
                  <!-- <td><?php echo $row['last_reading_value']?></td> -->
                  ...
                  <td>
                     <form method="post" class="d-flex align-items-center">
                        ...
                        <input type="number" name="dailyreading_<?php echo $row['id']; ?>" placeholder="Enter daily reading" required autocomplete="off">
                        <button class="btn btn-success btn-sm" type="submit" name="dailyreading_btn_<?php echo $row['id']; ?>">✓</button>
                     </form>
                     <?php
                        ...
                        include 'dbconfig.php';
                        if (isset($_POST['dailyreading_btn_' . $row['id']])) {
                        $dailyreading = $_POST['dailyreading_' . $row['id']];
                        $reqid = $_POST['request_id'];
                        $date = date('Y-m-d H:i:s');
                        
                        // Assuming 'meter_id' is the field in meter_reading to store the corresponding meter ID
                        $meter_id = $reqid; // Assuming the request ID corresponds to the meter ID
                        
                        // Construct the INSERT query for meter_reading
                        $insert_query = "INSERT INTO meter_reading (reading_value, reading_date, meter_id) VALUES ('$dailyreading', '$date', '$meter_id')";
                        
                        include 'dbconfig.php'; // Ensure the database connection is included
                        
                        // Execute the INSERT query
                        $insert_q = mysqli_query($conn, $insert_query);
                        
                        if ($insert_q) {
                        $update_meter_query = "UPDATE meters SET total_reading = total_reading + '$dailyreading' WHERE id = '$meter_id'";
                        $update_meter_q = mysqli_query($conn, $update_meter_query);
                                ...
                                <script type="text/javascript">
                                 window.location = "ecs_admin1.php"; // Redirect using JavaScript
                              </script>
                     ...
                     <script type="text/javascript">
                       ...
                       // window.location = "profile.php"; // Redirect using JavaScript
                     </script>
                     <?php
                      exit(); // Stop further execution
                        }
