...
$id = (int)($_GET['id'] ?? 0);
if ($id <= 0) {
    header('Location: transport_guard_record_list.php');
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax']) && $_POST['ajax'] === '1') {
    ...
    $stmt = $conn->prepare("UPDATE transport_guard SET transport_name = ?, status = ? WHERE id = ?");
    if (!$stmt) {
        echo json_encode(['ok' => false, 'msg' => 'Update failed!']);
        exit;
    }

    $stmt->bind_param("ssi", $transport_name, $status, $rid);
    $exec = $stmt->execute();
...
// Load record for GET
$stmt = $conn->prepare("SELECT id, transport_name, status FROM transport_guard WHERE id = ?");
                        ...
                        <form class="form pb-3" method="POST" autocomplete="off" id="guardEditForm">
                            <div class="card shadow">
                                <div class="card-header bg-header text-white d-flex justify-content-between align-items-center">
                                    <h6 class="mb-0">Edit Transport</h6>
                                ...
                                <div class="card-body">
                                    ...
                                    <div class="row g-3">
                                        ...
                                        <div class="col-md-8">
                                            ...
                                            <input type="text" name="transport_name" id="transport_name" maxlength="150"
                                                value="<?php echo htmlspecialchars($record['transport_name']); ?>">
                                        ...
                                        <div class="col-md-4">
                                            ...
                                            <select name="status" id="status">
                                                <option value="1" <?php echo ((int)$record['status'] === 1 ? 'selected' : ''); ?>>Active</option>
                                                <option value="0" <?php echo ((int)$record['status'] === 0 ? 'selected' : ''); ?>>Inactive</option>
                                            </select>
                                    ...
                                    <div class="text-center mt-4">
                                        ...
                                        <a href="transport_guard_record_list.php" class="btn btn-secondary btn-cancel">
                                            Cancel
                                        </a>
                                    </div>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            function validateName() {
                ...
                if (!v) {
                    setError(nameEl, nameError, "Transport name is required.");
                    return false;
                }
                if (v.length > 150) {
                    setError(nameEl, nameError, "Max 150 characters.");
                    return false;
                }
            ...
            function validateStatus() {
                ...
                if (!(v === "0" || v === "1")) {
                    setError(statusEl, statusError, "Invalid status.");
                    return false;
                }
