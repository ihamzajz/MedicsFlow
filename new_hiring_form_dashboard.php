...
if ($qRaw !== '') {
  ...
  $whereParts[] = "(
      ...
      OR reason_of_request LIKE '{$like}'
      OR emp_name LIKE '{$like}'
      OR emp_code LIKE '{$like}'
      OR designation LIKE '{$like}'
      OR required_education LIKE '{$like}'
      OR salary_range LIKE '{$like}'
      OR job_type LIKE '{$like}'
      OR job_responsibilities LIKE '{$like}'
      OR hod_status LIKE '{$like}'
      OR ceo_status LIKE '{$like}'
      OR hr_status LIKE '{$like}'
      OR hod_date LIKE '{$like}'
      OR ceo_date LIKE '{$like}'
      OR hr_date LIKE '{$like}'
      OR hod_msg LIKE '{$like}'
      OR ceo_msg LIKE '{$like}'
      OR hr_msg LIKE '{$like}'
      OR hod_comment LIKE '{$like}'
      OR ceo_comment LIKE '{$like}'
      OR hr_comment LIKE '{$like}'
    )";
}
...
<head>
  ...
  <title>New Hiring - Dashboard</title>
  ...
  <style>
    body { font-family: 'Poppins', sans-serif !important; background: #c7ccdb !important; }
    ...
    .btn { font-weight: 500 !important; font-size: 13px !important; }
    th { white-space: nowrap; font-size: 11.5px; border: none !important; background-color: #3a506b !important; color: white !important; font-weight: 600; }
    .table tbody td { font-size: 11px; border: none !important; font-weight: 400; vertical-align: top; }
    tr { border-bottom: 0.5px solid lightgray !important; }
    a { text-decoration: none !important; }
    label { font-size: 13px !important; }
    input { border: 0.5px solid #ced4da !important; font-size: 12px !important; }
    .btn-reset { background-color: #ced4da !important; }

    .mint-badge{
      display:inline-flex; align-items:center; gap:8px; padding:8px 12px;
      border-radius:999px; background:#d1fae5; color:#065f46; font-weight:700;
      font-size:12px; border:1px solid #a7f3d0; white-space:nowrap; line-height:1;
    }
    .mint-badge i{ color:#0f766e; }

    .btn-report{
      font-size:11px; font-weight:600 !important; color:white; background-color:#7c3aed;
      padding:7px 10px !important; border-radius:3px !important; display:inline-block; white-space:nowrap;
    }

    .td-wrap{ max-width:520px; white-space:normal !important; word-break:break-word; overflow-wrap:anywhere; }

    .table-responsive.dt-wrap{
      max-height:90vh; overflow:auto; border-radius:14px; border:1px solid #d8dee6; background:#fff;
    }
    .table-responsive.dt-wrap table{ width:max-content; min-width:100%; border-collapse:separate; border-spacing:0; }
    .table-responsive.dt-wrap thead th{ position:sticky; top:0; z-index:5; }
    .table-responsive.dt-wrap::-webkit-scrollbar{ height:10px; width:10px; }
    .table-responsive.dt-wrap::-webkit-scrollbar-thumb{ background:#c7ced8; border-radius:999px; }
    .table-responsive.dt-wrap::-webkit-scrollbar-track{ background:#eef2f7; }
  </style>
</head>
        ...
        <div class="container-xxl">
          ...
          <div class="card shadow-sm border-0 mb-3">
            <div class="card-body d-flex align-items-center justify-content-between flex-wrap gap-2">
              <div>
                <h5 class="mb-0" style="font-weight:700;">New Hiring - Dashboard</h5>
              </div>
                    ...
                    ?>
                        <tr id="row_<?php echo $rid; ?>">
                          <td style="white-space:nowrap;">
                            <a href="new_hiring_form_dashboard_report.php?id=<?php echo $rid; ?>" class="btn-report">
                          ...
                          <td class="td-wrap"><?php echo htmlspecialchars((string)($row['reason_of_request'] ?? '')); ?></td>

                          <td><?php echo htmlspecialchars((string)($row['emp_name'] ?? '')); ?></td>
                          <td><?php echo htmlspecialchars((string)($row['emp_code'] ?? '')); ?></td>
                          <td><?php echo htmlspecialchars((string)($row['designation'] ?? '')); ?></td>

                          <td class="td-wrap"><?php echo htmlspecialchars((string)($row['required_education'] ?? '')); ?></td>
                          <td><?php echo htmlspecialchars((string)($row['salary_range'] ?? '')); ?></td>
                          <td><?php echo htmlspecialchars((string)($row['job_type'] ?? '')); ?></td>
                          <td class="td-wrap"><?php echo htmlspecialchars((string)($row['job_responsibilities'] ?? '')); ?></td>

                          <td style="white-space:nowrap;"><?php echo htmlspecialchars((string)($row['hod_status'] ?? '')); ?></td>
                          <td style="white-space:nowrap;"><?php echo htmlspecialchars((string)($row['ceo_status'] ?? '')); ?></td>
                          <td style="white-space:nowrap;"><?php echo htmlspecialchars((string)($row['hr_status'] ?? '')); ?></td>

                          <td style="white-space:nowrap;"><?php echo htmlspecialchars((string)($row['hod_date'] ?? '')); ?></td>
                          <td style="white-space:nowrap;"><?php echo htmlspecialchars((string)($row['ceo_date'] ?? '')); ?></td>
                          <td style="white-space:nowrap;"><?php echo htmlspecialchars((string)($row['hr_date'] ?? '')); ?></td>

                          <td class="td-wrap"><?php echo htmlspecialchars((string)($row['hod_msg'] ?? '')); ?></td>
                          <td class="td-wrap"><?php echo htmlspecialchars((string)($row['ceo_msg'] ?? '')); ?></td>
                          <td class="td-wrap"><?php echo htmlspecialchars((string)($row['hr_msg'] ?? '')); ?></td>

                          <td class="td-wrap"><?php echo htmlspecialchars((string)($row['hod_comment'] ?? '')); ?></td>
                          <td class="td-wrap"><?php echo htmlspecialchars((string)($row['ceo_comment'] ?? '')); ?></td>
                          <td class="td-wrap"><?php echo htmlspecialchars((string)($row['hr_comment'] ?? '')); ?></td>
                        </tr>
                    <?php
                      }
                    } else {
                      echo "<tr><td colspan='30' class='text-muted fw-semibold p-3'>No record found!</td></tr>";
                    }
                    ?>
                  </tbody>
                </table>
