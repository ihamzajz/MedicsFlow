...
if (isset($_GET['export']) && $_GET['export'] === '1' && $tab === 'one') {
    ...
    header('Content-Disposition: attachment; filename=staff_allocation_export_' . date('Y-m-d_H-i-s') . '.csv');
...
<head>
    ...
    <title>Staff Allocation - Data Export Production</title>
    ...
    <style>
        ...
        /* ✅ Total badge (same style as your other pages) */
        .mint-badge {
            display: inline-flex;
            align-items: center;
            gap: 8px;
        ...
        /* ✅ Data panel like DataTable: 90vh scroll */
        .dt-wrap {
            max-height: 90vh;
            overflow: auto;
            border-radius: 12px;
    ...
    <script>
        ...
        document.querySelectorAll('button[data-bs-toggle="tab"]').forEach(btn => {
            btn.addEventListener('shown.bs.tab', (e) => {
                const id = e.target.id; // nav-one-tab / nav-two-tab
                const url = new URL(window.location.href);
