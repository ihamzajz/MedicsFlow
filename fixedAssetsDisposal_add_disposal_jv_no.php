...
<head>
    ...
    <title>Asset Disposal - Add Disposal JV Number</title>
...
<body>
        ...
        <div id="content">
            ...
            <a class="btn btn-dark btn-sm" href="assets_management_home.php" style="font-size:11px!important"><i class="fa-solid fa-arrow-left"></i> Home</a>
             ...
             <?php
                ...
                $select = "SELECT * FROM fixed_assets_disposal_form where jv_status = 'Pending' AND finance_status = 'Pending' ORDER BY date_of_application DESC";
    ...
    <form method="post" class="d-flex align-items-center">
        ...
        <input type="number" name="addjv_<?php echo $row['id']; ?>" placeholder="Add Jv No." required>
        <button class="btn btn-success btn-sm" type="submit" name="addjv_btn_<?php echo $row['id']; ?>">✓</button>
    </form>
    <?php
    include 'dbconfig.php';
    if (isset($_POST['addjv_btn_' . $row['id']])) {
        $addjv = $_POST['addjv_' . $row['id']];
        $reqid = $_POST['request_id'];

        $update = "UPDATE fixed_assets_disposal_form SET disposal_jv_no = '$addjv', jv_status = 'Approved' WHERE id = '$reqid'";
        $update_q = mysqli_query($conn, $update);

        if ($update_q) {
            ?>
            <script type="text/javascript">
                window.location.href = "fixedAssetsDisposal_add_disposal_jv_no.php";
            </script>
            <?php
        } else {
            ?>
    ...
    ?>
                        ...
                        <td><?php echo $row['id']?></td>  
                          ...
                          <td><?php echo $row['finance_status']?></td>
