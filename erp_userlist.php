...
if ($be_depart === 'super') {
  ...
  $name_safe = mysqli_real_escape_string($conn, $fullname);
  $whereParts[] = "name = '{$name_safe}'";
}
...
if (($hasFrom && !$hasTo) || (!$hasFrom && $hasTo)) {
  ...
  if (!$dfOk || !$dtOk) {
    ...
    if (strtotime($dateTo) < strtotime($dateFrom)) {
      ...
      // ✅ If your column is DATETIME, this safely covers whole days
...
<head>
  ...
  <title>ERP Access - Request History</title>
        ...
        <div class="container-xxl">
          ...
          <div class="card shadow-sm border-0 mb-3">
            <div class="card-body d-flex align-items-center justify-content-between flex-wrap gap-2">
              <div>
                <h5 class="mb-0 box1-heading">ERP Access - Request History</h5>
              </div>
                  ...
                  <tbody>
                    ...
                    if ($data) {
                      while ($r = mysqli_fetch_assoc($select_q)) {
                        ...
                        // ✅ show dd-mm-yyyy (filter still uses DB date)
                        $dateShow = fmt_ddmmyyyy($r['date'] ?? '');
                    ?>
                        <tr id="row_<?php echo $rid; ?>">
                          <td style="white-space:nowrap;">
                            <a href="erp_userlist_details.php?id=<?php echo $rid; ?>" class="btn-details">
