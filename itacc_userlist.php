...
/* =========================================================
   ✅ Server-side Pagination + Search + Date Range (SAME LOGIC)
========================================================= */
$perPage = 50;
...
/* Base WHERE: show all if super else show only logged-in username */
$whereParts = [];
if ($be_depart === 'super') {
  $whereParts[] = "1=1";
} else {
  $username_safe = mysqli_real_escape_string($conn, (string)$username);
  $whereParts[] = "username = '{$username_safe}'";
}

/* Search (it_accessories columns) */
if ($qRaw !== '') {
  $q = mysqli_real_escape_string($conn, (string)$qRaw);
  $like = "%{$q}%";
  $whereParts[] = "(
...
if (($hasFrom && !$hasTo) || (!$hasFrom && $hasTo)) {
  ...
  if (!$dfOk || !$dtOk) {
    ...
    if (strtotime($dateTo) < strtotime($dateFrom)) {
      ...
      $df = mysqli_real_escape_string($conn, (string)$dateFrom);
      $dt = mysqli_real_escape_string($conn, (string)$dateTo);
...
<head>
  ...
  <title>IT Accessories - Request History</title>
  ...
  <style>
    ...
    /* =========================================================
       ✅ SAME TABLE FIXES (like your workorder list)
    ========================================================= */
    ...
    /* wrap only where td-wrap is used */
        ...
        <div class="container-xxl">
          ...
          <div class="card shadow-sm border-0 mb-3">
            <div class="card-body d-flex align-items-center justify-content-between flex-wrap gap-2">
              <div>
                <h5 class="mb-0 box1-heading">IT Accessories - Request History</h5>
              </div>
                ...
                <table class="table table-hover align-middle mb-0" id="myTable">
                  ...
                  <tbody>
                    ...
                    if ($data) {
                      while ($row = mysqli_fetch_assoc($select_q)) {
                        ...
                        if (!empty($row['it_date'])) {
                          try { $itDateOnly = (new DateTime($row['it_date']))->format('d-m-Y'); }
                          catch (Exception $e) { $itDateOnly = ''; }
                        }
                    ...
                    ?>
                        <tr id="row_<?php echo $rid; ?>">
                          ...
                          <td class="td-wrap">
                            ...
                            <?php if ($itDateOnly !== '') { ?>
                              <br><span style="color:#64748b; font-weight:700;"><?php echo htmlspecialchars($itDateOnly); ?></span>
                            <?php } ?>
                          </td>
                    ...
                    <?php
                      ...
                      echo "<tr><td colspan='7' class='text-muted fw-semibold p-3'>No record found!</td></tr>";
                    }
                    ?>
                  </tbody>
                </table>
